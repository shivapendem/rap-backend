...
class ConsultantAssignmentService:
    ...
    async def update_consultant(
        ...
        # NOTE: sync_base_resume_text(db, consultant) used to also be
        # called here, before ConsultantRepository.update(). Removed —
        # it duplicated the identical call a few lines below (right before
        # commit), which was firing build_base_resume_content() twice and,
        # more importantly, launching the fire-and-forget background DOCX
        # regeneration task twice per admin save. Keeping only the later
        # call (post-refresh, right before commit/log_action) preserves
        # the original fix's intent with no duplicate work.
