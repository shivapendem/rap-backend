...
class RequirementConsultantMatch(Base):
    ...
    status = Column(Text, nullable=False, default="MATCHING")
    # Independent of `status` on purpose — resume progress and match
    # eligibility used to share one overloaded status column (the exact
    ...
    VALID_RESUME_STATUSES = {None, "RESUME_GENERATED", "READY_TO_APPLY"}
    ...
    @validates("resume_status")
    def validate_resume_status(self, key, value):
        if value not in self.VALID_RESUME_STATUSES:
            raise ValueError(f"resume_status must be one of {sorted(str(v) for v in self.VALID_RESUME_STATUSES)}, got '{value}'")
        return value
