...
def role_from_subject(subject: str) -> Optional[str]:
    ...
    # generic filler OR an eligibility/employment-type restriction
    # clause instead, falling back to segment [0] if literally every
    # segment looks generic (same as the unconditional behavior before
    # this fix).
    # BUG FIX ("role: 'Direct'" from "Direct Client ::Software Quality
    # Assurance Engineer III :::Lake Forest, IL" — confirmed on a real
    # requirement row): "::" (two or more colons) is just as common a
    # segment separator in real recruiter subjects as "|" or "//", but
    # wasn't recognized at all. A single colon is deliberately NOT
    # included here -- that's the ordinary "Label: Value" separator used
    # everywhere else in this file, and splitting on it would break far
    # more than it fixes.
    ...
    _MARKETING_PHRASES = (
        ...
        # BUG FIX ("role: 'Have Interview Slots - RPA UI Path Developer'"
        # — confirmed on a real requirement row): "Have Interview Slots"
        # is the same class of urgency-marketing opener as "Immediate
        # Interviews"/"Urgent" above, just a different common phrasing
        # ("we already have slots booked, submit fast") — wasn't in the
        # recognized vocabulary at all, so it survived as a literal
        # prefix glued onto the real title.
...
def _extract_skill_tokens(sentence: str) -> List[str]:
    ...
    for tok in tokens:
        tok = tok.strip().strip('.')
        # BUG FIX ("skills: ['- *Kinaxis RapidResponse*', '*Supply
        # Planning*', ...]" — confirmed on a real requirement row): a
        # bulleted skills list individually markdown-emphasizes each
        # item ("*Kinaxis RapidResponse*"), and the leading "- " bullet
        # dash sometimes survives a fallback split too. Neither is
        # stripped anywhere upstream (normalize_text()'s own asterisk
        # fix only targets label WORDS, deliberately never touching
        # emphasis around ordinary content like a real skill name -- see
        # its own comment). Strip a leading bullet dash and any leading/
        # trailing "*" run directly off each individual token here,
        # after splitting, so the emphasis markup never ends up as part
        # of the stored skill value.
